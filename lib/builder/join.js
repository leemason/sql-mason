...
module.exports = {
    ...
    getJoinOns: function(){
        return this._builder.joinOns || [];
    },

    on: function(key, operator, value){
        this._builder.joinOns = this._builder.joinOns || [];

        this._builder.joinOns.push({
            key: key,
            operator: operator,
            value: value,
            connector: (arguments.length == 4) ? arguments[3] : 'and'
        });

        return this;
    },

    orOn: function(key, operator, value){

        return this.on(key, operator, value, 'or');
    },

    andOn: function(key, operator, value){

        return this.on(key, operator, value, 'and');
    },
